/* test_auth.js testitiedostossa tehdään http-pyyntöjä supertestillä ja
   testataan autentikaation toimintaa. Huomaa että testit menevät läpi vasta
   valmiissa sovelluksessa. Ne eivät mene läpi vielä tehtävän alkutilanteessa.
*/

//const expect = require('chai').expect; // tarpeeton koska supertest sisältää oman expectin
import request from 'supertest'; // supertest-kirjastoa käytetään pyyntöjen (request) testaukseen
import app from '../app.js';

describe('Test authentication', (done) => {
  const user = {
    email: 'foo@bar.com',
    pass: 'qwerty',
  };
  //Tehdään login ennen testiä
  const authenticatedUser = request.agent(app);
  before((done) => {
    authenticatedUser
      .post('/')
      .send(user)
      .expect(302) // 302 on redirect. Supertest sisältää oman expect assertin
      .expect('Location', '/sivu1')
      .end(done);
  });

  it('should return a 200 response if the user is authenticated', (done) => {
    authenticatedUser
      .get('/sivu3') // vaihda tähän sivu1 tai sivu2, niin menee läpi alkutilanteessa
      .expect(200) // supertest expect
      .end(done);
  });
});
